...
const LoginSignup = () => {
    ...
    try {
      ...
      if (responseData.success) {
        ...
        if (responseData.errors) {
          ...
          alert("Login failed. Please try valid email and password.");
        }
      }
    } catch (error) {
      console.error('Error during login:', error);
  ...
  const signup = async () => {
    ...
    try {
      ...
      alert("Existing user with same details.Please fill the details and try again.");
    }
    // try {
    //   const response = await fetch('http://localhost:4000/signup', {
    //     method: 'POST',
    //     headers: {
    //       Accept: 'applicatio/form-data',
    //       'Content-Type': 'application/json',
    //     },
    //     body: JSON.stringify(formData),
    //   });
